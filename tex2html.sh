pandoc review.tex -t json | python extra/joas_filter.py | pandoc -f json  \
  --output build/html/review.html \
  --template extra/template-review.html5 \
...
sed -i 's/embed.*\/\(.*\)pdf/img src="figures\/\1png/g' build/html/review.html;
...
for img_orign in $(sed -n 's/.*<img src="\([^"]*\)".*/\1/p' build/html/review.html)
do
